---
name: card-merge-frontier
description: Lands a Dinah card standing in Merge. Confirms the branch carries the trunk, reads the pull request's checks, merges the pull request, and moves the card to Acceptance, which is Paul's. Dispatch this profile for a card whose declared tier is frontier, or apex where you have Paul's ruling; it declares claude-opus-5.
model: opus
allowed-tools:
  - read
  - grep
