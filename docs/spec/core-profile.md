...
records are for the workbench to define; the core takes three things here
and no more, which are the item's existence, whether it has been answered,
and the column it names.

An item that is not yet resolved may cause a tool to refuse a claim or a
move on that ground. This profile itself requires two such refusals. Section
6.3 requires a claim to be refused by an unresolved item naming no column the
workbench declares, and section 6.4 requires a move to be refused at a column
a workbench has marked as needing the item resolved before a card enters it. A
workbench that refuses either verb under a condition this profile does not
itself require does so under a rule of its own rather than under a rule this
section states, and CORE-ITEM-3 fixes the one name every such refusal reports
so a caller meeting it from any conforming tool reads the same thing whichever
verb produced it.
...
The seventh row reads the items the card carries against the columns the
workbench declares. An item that names a column is an item some column of the
workbench may one day stop the card for, whether the workbench marks that
column under CORE-GATE-1 or under a layer of its own, so the claim leaves it
to that column. An item that names no column the workbench declares is an item
no column can ever be asked about, so the claim is the last place anything can
refuse over it.
...
7  every unresolved item the card carries names a declared column   unresolved-item
```
...
[CORE-CLAIM-10] A tool MUST refuse a claim on a card carrying a structured item that is not resolved and naming no column the workbench declares, reporting the refusal name `unresolved-item`.
...
The column says that it holds and nothing more. Which items hold there
follows from which of a card's items name the column, so no kind of item is
enumerated on the column and none is treated differently from another when
the hold is applied. A workbench that wants only one kind held at a column
files only that kind against it.

The way through is the way through a capacity limit, and for the reason
given there: a control nobody can ever set aside is worked around where
nothing sees it. The operator marks the request as an override, the move is
recorded as an override rather than passing as an ordinary move, and every
other owner is refused whatever they carry.

[CORE-GATE-1] A workbench MAY mark a column as requiring a named structured item to be resolved before a card enters it.

[CORE-GATE-3] A tool MUST refuse a move into a column so marked, reporting the refusal name `unresolved-item`, while the card carries the item that column names and that item is not resolved, except where the request carries an override marker and the owner asking is the operator of the workbench.
...
| Behaviour attached to a reference between cards [dependency ordering, ready-work listing] | out | The core would gain enforcement whose meaning each workbench sets differently, and what a workbench should do about a reference is exactly the judgement that differs between them. A tool that wants one card to hold another back declares a layer and refuses under that layer's own name, which CORE-LINK-5 leaves it free to do. One implementation withholds a card from its own selection while a card it is linked to has not started or finished, under a layer in which each workbench names the link kinds that hold and which end they hold, and refuses no act on that ground. | A relationship must refuse an act, such as one card holding another back. | |
...
| A column's own declared destination for a card the work there refuses | out | The core already has an ordinary move to any column, which is what a rejection is made of, and the extra declaration would add a name for a destination without adding a rule the tool enforces beyond what CORE-STATE-8 already permits. The hosted product's board configuration already carries a column-level directive naming where a pushed-back card lands, which is the same shape one layer up; whether that directive is the same concept closely enough to satisfy the reopen condition is a question for whoever reviews this promotion path next, and this row does not answer it. | An implementation's own form of this has run on real workbenches long enough to be worth copying, and a second implementation needs to read it rather than merely preserve it. | |
| A read naming the column a pull standing at a given column would carry a card into | out | The act this would describe is itself outside the core. The column kind a pull carries cards through is ruled out by the row for a column that buffers for a downstream column, so an implementation carrying no such kind has no such destination to name and owes nothing here. Where a layer does carry the kind, the destination follows from what the core already fixes, which is the ordered list CORE-STATE-6 makes the flow, read together with the layer's own rule about which columns take work up, so publishing the answer states no rule that was not already stated and gives a reader no obligation it could hold a tool to. What it buys is that the walk is written once in the tool instead of once in every reader, and a reader that derives its own copy carries a rule that goes stale the next time the walk changes. One implementation publishes it for that reason and considers it worth the key. A second implementation owes it only if it wants its own readers to stop deriving the answer, and a reader meeting a workbench that publishes nothing here works the answer out from the ordered list and the kinds it already holds. | The column kind a pull carries cards through enters the core, at which point where a pull lands stops being a layer's question and becomes one the core itself has to answer. | |
...
| DOC-VER-11 | must | document | While the document's major number is 0, every identifier of the prior revision appears in the extraction of a minor revision, unless that revision's changelog entry marks it retired. |
| DOC-VER-12 | must | document | Once the document's major number has reached 1, every identifier of the prior revision appears in the extraction of a minor revision. |
...
| DOC-VER-8 | must | document | While the document's major number is 0, a revision whose difference falls under none of DOC-VER-1, DOC-VER-11, DOC-VER-4, DOC-VER-7, or DOC-VER-9 carries an incremented minor number and an unchanged major number. |
...
| DOC-VER-10 | must | document | Once the document's major number has reached 1, a revision whose difference falls under none of DOC-VER-1, DOC-VER-12, DOC-VER-4, DOC-VER-7, DOC-VER-8, or DOC-VER-9 carries an incremented major number. |
...
| CORE-CLAIM-10 | must | tool | A claim on a card carrying an unresolved structured item that names no column the workbench declares is refused with `unresolved-item`. |
...
under the same condition. DOC-VER-2, retired. DOC-VER-11, introduced,
...
### 0.11, channel `dev`, 2026-09-04

Identifiers affected: CORE-CLAIM-9, introduced. No other identifier in the
section 11 index is affected, and this entry retires none.

This entry completes the claim-time half of the structured-item model 0.10
promoted. Section 10's "Structured items on a card recording judgements" row
named a pending open question refusing a claim as part of what justified
promoting it; this entry supplies the statement that actually requires the
refusal, using the item model and the refusal name 0.10 already fixed.

Consequence for a caller. A tool conforming to `dinah-core 0.11` must now
refuse a claim on a card carrying an unresolved structured item, reporting
`unresolved-item`. Unlike 0.10's own two CORE-GATE statements, this is not
something a workbench opts into: a tool that stores structured items at all,
and carries one unresolved on a card, changes behaviour under 0.11 whether
or not it declares anything, because CORE-CLAIM-1 through CORE-CLAIM-8
already apply to every claim without a workbench's say-so and CORE-CLAIM-9
is one of their number. A tool that stores no structured item sees no change, since
...
### 0.13, channel `dev`, 2026-09-09

Identifiers affected: CORE-JSON-9, retired. CORE-JSON-10, introduced, carrying
CORE-JSON-9's permission with `gate_items` added to the set of members a column
object may carry. CORE-GATE-2, retired. CORE-GATE-3, introduced, carrying
CORE-GATE-2's demand with an exception for the operator's override marker.
CORE-GATE-4, introduced: a move into a gated column carrying that marker is
admitted and recorded as an override. No other identifier in the section 11
index is affected.

This revision also grows the ordered check table of section 6.4 by one row, the
ninth, which is the gate CORE-GATE-3 states. The table's own order is under
DOC-ORDER-2, so the revision takes a minor increment on that ground as well as
on the statement changes above, and a tool reading its checks off that table
finds the gate row inside the profile's own list rather than after it.

A gate that nobody could ever set aside would be worked around outside the
tool, where nothing records it, which is the reasoning section 6.4 already
gives for the capacity limit. CORE-GATE-2 as published admitted no way through,
so a tool offering the operator one departed from it. The pair replacing it
puts the gate on the same footing as the capacity limit: the refusal stands for
every owner who carries no marker, the operator's marker admits the one move it
is passed on, and the act is recorded as an override rather than passing as an
ordinary move.

Consequence for a caller. A tool reading or writing the interchange form may
now carry a `gate_items` member on a column object, and a tool that does not
recognise it and has not read this revision sees it travel through as an
unrecognized member under CORE-JSON-7, unaffected. A caller conforming to an
earlier revision was entitled to be refused at a gated column whatever it
carried; under `dinah-core 0.13` a caller that is the workbench's operator and
carries an override marker may find the move admitted instead, and it learns
that this happened from the act's own record rather than from the refusal it
did not get. A tool that offers no override at all still conforms, because
...
### 0.15, channel `dev`, 2026-09-14

Identifiers affected: CORE-CLAIM-9, retired. CORE-CLAIM-10, introduced,
carrying CORE-CLAIM-9's refusal narrowed to an unresolved item that names no
column the workbench declares. CORE-ITEM-4, introduced: a structured item may
name a column of the workbench that carries it. No other identifier in the
section 11 index is affected.

The difference is a minor increment under DOC-VER-7, which classifies a
revision that has lost an identifier while the document's major number is
still 0, and DOC-VER-11 is satisfied because the changelog entry marks the
lost identifier retired. CORE-CLAIM-9 could not be reworded where it stood,
because DOC-VER-4 requires the texts an identifier carries across two
revisions to match unless the keyword weakened, so the narrowed rule takes an
identifier of its own on the pattern 0.13 used for CORE-GATE-2 and
CORE-GATE-3.

CORE-CLAIM-9 refused a claim over any unresolved item a card carried, and an
item already names at most one column. The two rules together refused the
claim at every column ahead of the column meant to settle the item, and at
that column too, so the station whose job was to answer the item could not
take the card up. Section 5.9 also gains CORE-ITEM-4, because the narrowed
rule rests on an item naming a column and the profile had so far stated that
relation only inside CORE-GATE-1's permission.

Consequence for a caller. A caller conforming to an earlier revision was
entitled to a refusal it no longer gets: a claim on a card carrying an
unresolved item is now admitted unless that item names no column the workbench
declares. Nothing that was admitted before is refused now, because every item
