...
10. **The terminal is lent by ending the program and starting another.** `edit` hands the terminal to the editor through the program's own end and start, so nothing rests on how Bubble Tea behaves while paused (dinah-623/decisions/5). A paste into the command line is taken only when it holds one line (dinah-623/decisions/7).
