...
The surfaces document and the workbench's own standing text disagree on where a terminal UI would live. The surfaces document says every head lives in the one binary, as `dinah mcp` and the planned `dinah ui` do. The workbench says the command-line tool stays tiny with no board of its own. The operator's second ruling in section 9 settles it in favour of the one binary, and the sixth ruling then replaced that for this head alone: the terminal UI is its own program, `dinah-tui`, which `dinah tui` starts.

### 8.1 What the terminal UI reaches, and the check that holds it to the CLI

The operator ruled that the terminal UI corresponds one to one with the CLI (dinah-623). Every entry of the command table carries a terminal class beside its group, so the author of a new command meets the question on the line that makes the command exist. A command classed `direct` is performed from a key of its own or from the actions menu; one classed `line` is reached by typing it at the terminal UI's command line or through a key binding; and one classed `absent` cannot run inside a live screen, for a reason the catalog carries. The six absent commands are `mcp`, `lsp`, `serve`, `ui`, `completion` and `tui`. Every command that writes to a card is direct, and the reads the operator uses most, `next`, `status`, `search`, `query`, `changes` and `whoami`, have keys of their own.

Seven tests make up the check, and each is derived from the command table, the parameter table, the head's act table or its actions menu, never from source text. They fail on a command with no class, a card verb reached by neither a key nor the menu, a menu entry the workbench would refuse, a direct command the head does not reach, a command typed at the line that never reaches its run function, an absent command or a waiting flag that is not refused, and a command that resolves a setting afresh. The check cannot see a flag or a command that waits: a new flag that blocks until something happens, or a command that never returns, can be classed `line` and every test still passes, so whoever adds one names it in the table of refused flags.

The actions menu, on `x`, lists every verb that can act on the selected card that the workbench would accept, in the command table's order. The library answers what it would accept through `OfferActs`, and every member of that answer comes from the check function the verb itself calls, so an entry the menu lists is an act the verb would pass every check it runs before reading what the person types. The same answer drives item mode, on `i`, which lists the card's questions, decisions and criteria carrying an act the owner may take, and the operator-only rules of dinah-472 reach the screen as absent keys.

The command line, on `:`, is shared with the jump. A line whose first word names neither a command nor an alias is a jump, and `@` forces one. A command runs through the CLI's own parser and dispatch, in the same process, on a line session built by one constructor. That constructor pins the workbench, the actor, the declared agent, the language and the glyph set to the values the head started with, by feeding them into the places the CLI already reads each setting from: the workbench as the top rung of discovery, the pinned settings as an overlay on the configuration file, and the identity and the renderer as copied fields. Tab completion reads through the same session. The workbench fed in as the top rung is the head's own and was never given by the person at the command line, so `init` and a walk from `--root`, which refuse a workbench named with `--workbench` or `DINAH_WORKBENCH` as a contradiction of what they were asked to do, do not read the pin as one. `edit` hands the terminal to the editor by ending the program, running the editor on the terminal given back, and starting a new program over the same model, which measures the window afresh.

A key binding is a setting in the user's own configuration, `tui.key.<key>` with an optional `tui.label.<key>`. It runs its command line through the command line's own path, with `$card`, `$column` and `$view` each substituted into the one word it stands in. A binding can never take a key the screen reads itself, and the footer hides a binding whose command is a single act on the selected card that the workbench would refuse.

Two refusals belong to this part of the design. `dinah.not-in-tui` refuses a line that cannot run inside the screen, naming the word the person typed and a machine token saying why. `dinah.invalid-key-binding` refuses a `tui.key` setting the screen cannot use, naming the setting and a defect token.

## 9. Decisions

The operator ruled on all four decisions on 2026-09-25, and on a fifth that followed from the fourth. The sixth and seventh were ruled on 2026-09-26, while the terminal UI was built, and the eighth to the eleventh on the same day, when the terminal UI was made to reach every command.
...
6. **The terminal UI is its own program.** The operator ruled on 2026-09-26 (dinah-603/questions/4) that the terminal UI builds as `dinah-tui`, from the same package under the `tui` build tag, shipped beside `dinah`, and that `dinah tui` starts it. The code review had measured what linking Bubble Tea into `dinah` cost every command. On Windows `dinah version` went from about 73 ms to about 95 ms, averaged over 20 runs, and total package initialisation went from 37 ms to 63 ms. `go-runewidth` filled a 2.2 MB table at start-up on every platform, and on Windows `atotto/clipboard` loaded `user32.dll` at start-up. Agents run `dinah` constantly and none of that is theirs to pay, so this head alone departs from the one binary. `dinah tui` finds `dinah-tui` beside itself, then beside the path its links resolve to, then on `PATH`, and refuses with `dinah.tui-missing` when it finds none. `dinah-tui` refuses with `dinah.tui-skew` when the `dinah` that started it is another build. Help and completion stay in `dinah`, so neither needs `dinah-tui` installed.
7. **On Windows the terminal UI relies on bracketed paste and one named residual, and nothing else Microsoft leaves undocumented.** The operator ruled on how the terminal UI writes to the Windows console on 2026-09-26, at dinah-603/comments/23, dinah-603/questions/5 and dinah-603/comments/28. Bubble Tea is given `TERM=xterm` and an output that is not a terminal file, and every carriage return and line feed is rewritten as the cursor movements Microsoft's "Console Virtual Terminal Sequences" page lists, so every sequence the renderer can write there is on that page. Each frame is cut into pieces of at most 8,000 UTF-16 code units, only between whole escape sequences and whole characters, and each piece goes to the console in one `WriteConsoleW` call through the UTF-16 console writer the rest of Dinah writes through. What remains undocumented is bracketed paste, which the page does not list, and one residual: a console could report writing fewer characters than it was given, in the middle of a sequence, and Microsoft's `WriteConsole` page neither promises that it will not nor says when it would. Two mitigations bound that residual. The writer reads the count the console reports, writes the rest of the piece, and has the next frame drawn in full, so a frame the split drew wrongly lasts one frame. Ctrl+L draws the whole screen again on demand, on every platform. Section 5.4's account of why `--watch` uses the classic console functions describes the watch and stands.

8. **Every card verb is reached from the terminal UI directly.** The operator ruled on 2026-09-26 (dinah-623/comments/1 and /6) that the terminal UI corresponds one to one with the CLI, that anything it does not do directly is reachable from a command line inside it, and that every verb acting on a card is a direct act. The actions menu makes every card verb direct on dinah-623, and dinah-624 gives a few of them keys of their own. The most-used reads got keys of their own (dinah-623/questions/2), and a filtered palette on `:` and command history were filed as dinah-628 and dinah-629.
9. **A line runs in the head's own process, pinned to the head's start values.** A command typed at the command line, run by a key binding or by a read key, and Tab completion all read the workbench, the identity, the language and the glyphs the head started with (dinah-623/decisions/4). A line runs synchronously, as a key act does, and the two commands that wait by design are refused rather than run (dinah-623/decisions/3). An item act carries no revision, because the item verbs check the item's own state under the card's lock (dinah-623/decisions/2).
10. **The terminal is lent by ending the program and starting another.** `edit` hands the terminal to the editor through the program's own end and start, so nothing rests on how Bubble Tea behaves while paused (dinah-623/decisions/5). Every frame asks for bracketed paste on, because Bubble Tea documents a view's `DisableBracketedPasteMode` as turning the mode off for that view. Its renderer starts ticking before a program's Init returns and may draw its own zero View, which asks for the mode on, ahead of the head's first frame, and a frame asking for the mode off then switched it off for the rest of the session once a lend had restarted the program (dinah-623/criteria/52). A paste into the command line is taken only when it holds one line (dinah-623/decisions/7).
11. **Key bindings come from the user's settings alone, and never shadow the screen's own keys.** A binding set shared through the workbench would let whoever edits the workbench decide what a colleague's keypress writes under that colleague's name (dinah-623/decisions/9), and no override lets a binding take a key the footer offers only when the workbench would accept its act (dinah-623/decisions/10).
