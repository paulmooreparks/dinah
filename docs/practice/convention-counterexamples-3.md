...
## Reading a slice documented as ordered, at a position, without checking whether the code that built it actually sorted

Caught at Agent Code Review on dinah-573, 2026-09-22, by a reviewer tracing a criterion's exact wording against the code rather than reading the criterion's test as proof of it. The feature is dinah prime's Reread member, meant to name the column of the earliest-arrival card among the cards the caller holds.

**Wrong:** `holding[0].Column`. `Primer.Holding`'s own doc comment says its members come "in arrival order," which is true only because AC2 requires it byte-identical to `Status.Holding`, and `Status.Holding` is built by appending each held card as `Bench.Cards()` yields it, with no sort call anywhere in the path. `Bench.Cards()` walks its directory with `os.ReadDir`, which Go sorts by filename, and a card's filename is its 12-hex random ID, which `bench.ByArrival`'s own doc comment states is deliberately meaningless as an ordering. For an actor holding one card the two orders cannot disagree, so the card's own test, which holds exactly one, stayed green through the whole implementation and both review passes that read it. For an actor holding two or more, `holding[0]` is the lexically-smallest-ID held card, not the earliest-arrival one, whenever those diverge, which is the ordinary case for random IDs.

**Right:** compute the ordering the claim needs, explicitly, from the values that actually carry it, and leave the documented-but-unsorted slice alone. `Library.Prime` now carries the held cards' own `*bench.Card` values alongside the `CardView`s `Holding` is built from, and searches that slice with `bench.ByArrival` for the one Reread needs, touching `Holding`'s own order not at all, since a second requirement (AC2, byte-identical to `Status.Holding`) pins that order to something Reread's own claim cannot be read out of.

**The test:** wherever a value is read at a fixed position from a collection whose contract states an order, ask what code path actually establishes that order, and read it rather than the sentence describing it. A comment or a field name asserting "in X order" is a claim about the writer, not a proof the reader can skip checking. The tell here was structural: the same collection served two claims (AC2's byte-identity to an unsorted build, and AC12's earliest-arrival-first), and the two are only reconcilable if the collection happens to already be sorted the second way, which it was not. Arm it with two or more members whose natural build order and whose claimed order disagree, engineered rather than hoped for: this corpus's own entry used `os.Rename` to force two held cards' IDs to sort opposite their arrival order, since nothing in the ordinary card-creation path lets a test choose a card's ID.

**Related:** "A test asserting the value the write happened to produce, where the write was the defect," in `convention-counterexamples-1.md`, is the sibling on the single-member side of this shape: there a test's own expectation was read off an unsorted or otherwise unproven build; here a criterion's own verifying test held only one member, so no build order could have contradicted it either way. Both are the same failure at different collection sizes: a test that cannot distinguish the right build from the wrong one is not verifying the claim it is attached to.

## A two-route recovery that consults the inference before the evidence

Caught at Agent Code Review on dinah-472, 2026-09-23, by a reviewer who took the implementer's own written doubt as the place to start executing. The feature is `dinah check --migrate-designations`, which carries a checklist item's answer from the designated comment's position to that comment's own identifier, and which has two ways of working out which comment an answer meant.

One route reads the card's journal: where a `commented` line stands immediately before the settling line and names the same item, the same actor and the same instant, the settling is the `--text` form, which mints a comment and designates it in one act, and the line carries that comment's identifier. The other resolves the stored position, where nothing has been archived, restored or deleted since the settling and so no position can have moved. The first is an inference about two journal lines. The second is evidence: the value on disk was correct when it was written and nothing since could have moved it.

**Wrong:** try the journal first and return unconditionally when it matches, then fall through to the position. The guard on the journal route rested on an asserted property, that one write stamps both lines from one reading of the clock so two lines carrying one instant are halves of one act. The stamp carries seconds. Two separate invocations inside one second carry one instant too, and nothing in the record separates them. The reviewer reproduced the loss in three commands: a comment carrying the answer, a second comment carrying an aside, and a settling naming the first, all inside one second, on a store where nothing had ever been removed and the stored position still reached the answer. The run reported `Converted 1 item from the history` and wrote the aside's identifier down. The item then cited words nobody had settled it with, the answer somebody meant was cited by nothing, the report called it a recovery rather than a guess, `dinah check` afterwards found no defect, and the rehearsal printed the same thing, so rehearsing first did not catch it either. The population this lands on is the ordinary rhythm of an agent session, where writing a comment and settling an item in the same second is what the work looks like.

**Right:** order the routes by the strength of what each rests on, and reach for the inference only where the evidence is gone. Ask first whether anything since the settling could have moved the stored position; where nothing could, resolve the position and stop. Consult the journal only for the items whose positions may have shifted, which is exactly the population the evidence no longer covers. The reproduction above then converts as undisturbed and the item keeps citing the answer it was settled with, and the case the journal route exists for, an item whose earlier comment was deleted afterwards, still reaches it and still recovers.

**The test:** wherever a function recovers one value two ways, write down what each way rests on before writing down which runs first, and put the stronger one first. An inference placed ahead of an evidenced reading wins on precisely the cases where the evidence was available and correct, which is the population the recovery was least needed for and the one where being wrong costs the most. Arm it by transposing the two blocks and watching a fixture built on the coincidence go red: plant the coincidence rather than hoping for it, and assert which value the caller ends up holding rather than which route the report named, because the label is not what a reader loses. The fixture has to engineer the timing rather than assume it, and it has to fail loudly rather than skip when it cannot: a case that quietly stands down on a slow machine is the same silence wearing a label.

**Related:** "A construct excluded from a guard's threat model by an asserted property nobody tested," in `convention-counterexamples-2.md`, is the same reasoning error one layer in: there a guard's threat model excluded a construct on a property no fixture supplied, and here a route's precedence rested on a stamp resolution no fixture exercised. The tell in both is a doc comment arguing that something is safe rather than merely likely, with no case beneath it that could tell the two apart. This card's own handoff invited reviewers to weigh the stamp test, which makes this a second sighting of the shape rather than a first.

## Text drawn under --watch without replacing its control characters, because the path it came from was not the one the cleaning lived on

Caught at Agent Code Review on dinah-288, 2026-09-25, by a reviewer who stored the hostile value and watched what reached a real terminal, rather than reading the test that claimed to cover it. The feature is `dinah view --watch`, which redraws a view in place by driving the terminal a row at a time. Two paths feed it. The board lays out its own cells and passes every card title, holder and column title through `withoutControls`. Every other view, the list layout and `--explain` on any layout, reaches the watch through `drawnText`, which captures what a one-shot draw prints.

**Wrong:** `drawnLine{text: cutText(line, width-1, ellipsis)}` for each line `drawnText` returned. The one-shot drawing writes stored titles exactly as the workbench holds them, which is right for a pipe and for a person running the command once. Under `--watch`, though, the same bytes go to a terminal the tool is positioning and colouring. A title carrying `ESC [2J` cleared a Windows console partway through a frame. A title carrying an OSC sequence set a Linux terminal's window title. A title ending in a cut-off UTF-8 sequence left consolewriter holding two bytes across the console calls that followed, so they came out as two replacement characters on the status row and pushed it past the window's edge. Every board test passed, because the board cleans at the source, and the test asserting "no escape reaches the console" drew only a board, from cards whose titles carried none.

**Right:** `drawnLine{text: cutText(withoutControls(line), width-1, ellipsis)}`. Clean at the boundary the hazard crosses, which is the point where any text reaches a terminal the tool is driving, rather than at the one source the author was thinking of. `withoutControls` is `strings.Map`, which also turns invalid UTF-8 into U+FFFD, so one call covers both kinds of bad bytes. The board's remaining inputs, the acting clause and a refused section's lines, go through it too.

**The test:** where one safeguard protects a sink, enumerate every path that reaches the sink and check each one, rather than checking the path the safeguard was written for. Arm it from the hostile side: store the value a terminal would act on, including an escape, BEL and a truncated multi-byte sequence, drive every path that can draw it, and assert on what reached the terminal seam, byte for byte. Then remove the cleaning from each path in turn and watch that path's case go red. A fixture whose data carries no hostile value proves only that clean data stays clean.

**Related:** "A test passes by avoiding the hard position" is the workbench memory this sighting repeats: the criterion's own test never drew the value that breaks the code. "A two-route recovery that consults the inference before the evidence," above, is the same shape seen from the other side: two routes to one result, where the property that made one route safe was never true of the other.

## One answer that reads the clock more than once, so a run across midnight selects on one day and draws on the next

Caught twice on dinah-605, 2026-09-25, at Agent Code Review, by a reviewer tracing each caller of `Bench.Today` rather than reading the function that computes a card's conditions. The feature gives a card start and due dates and derives conditions from them against today: overdue, due soon, not yet startable. `dinah query schedule:due_soon` selects on those conditions, and every listing prints them.

**Wrong:** `today := l.Bench.Today(l.Now())` inside `view`, the helper that builds one card's row. Each verb read today once to select its cards and then called `view` for every card it printed, and `view` read the clock again. Run across midnight in the workbench's zone, `dinah query schedule:due_soon` printed a card it had selected as due soon with a cell reading overdue, so the answer contradicted its own filter, and `dinah list` could draw two rows of one listing against two different days. The first sighting was the renderer counting "overdue by n days" from a clock read of its own; round 2 fixed that one surface and left the same second read in list, stored views, prime and next. No test caught either, because every fixture's clock stood still.

**Right:** read today once per request and pass it to everything that request selects or draws. `Library.today(req)` reads the clock the first time anything asks and keeps the answer on the request, `view` takes the day as a parameter, and the query parser, the start hold, each offer and each view section receive the same value. A head that repeats an act, such as a watch drawing frame after frame, builds a fresh request for each repetition so that no frame inherits a day an earlier frame read. The day also belongs to the workbench that read it. It is read in that workbench's zone, and a root walk hands one request to every workbench in turn, so the request records which workbench read the day and `today` reads the clock again when a different one asks. The first shape of this fix kept a bare date on the request, and a workbench in Auckland handed its day to one in Los Angeles on every `--root` walk but the one that happened to clear it.
...
## An output-set assertion over a run whose frames the renderer is free to diff, so the forbidden path is never reached

Caught at Agent Code Review on dinah-603, 2026-09-26, by a reviewer who ran the Windows configuration of the terminal UI a dozen times through a throwaway test rather than reading the test that claimed to cover it. The feature draws the terminal UI with Bubble Tea's renderer on the Windows console, where only the sequences Microsoft's "Console Virtual Terminal Sequences" page lists may be written. The head hid `TERM` from the renderer, and a test drove every mode through a headless run and refused any sequence outside a documented set, insert mode by name.

**Wrong:** `run := runTUIThrough(t, root, tuiSeam(t, strings.NewReader(keys), 100, 30))`, then a loop over every control sequence in `run.output` refusing `ESC [4h` and its siblings. Without a terminal type the renderer has no insert-character capability, so a row that gains cells in the middle is drawn by switching the console into insert mode, which the page does not list. Whether a run takes that branch depends on which two frames the renderer happens to diff, which depends on the timing of its own ticker. The sweep's frames never produced a row that gained cells in the middle, so it passed every time, and the reviewer's runs wrote `ESC [4h` in about one run in sixteen.

**Right:** build the frame pair that forces the path and wait on content rather than time between the two. `TestAnInsertedRunNeverUsesInsertMode` draws frame A with a row reading `abcdefgh`, waits until the output holds it, switches to frame B, whose row reads `abXYcdefgh`, waits until the output holds `XY`, and asserts on the bytes written for B alone. A companion test draws the same pair with `TERM` removed and requires `ESC [4h`, so the pair is shown to reach the forbidden branch when the capability is missing, and the configuration is shown to keep it away when it is present. Say in the test's comment what it does not prove: under the fixed configuration the renderer may draw the pair another way, and the claim that insert mode is unreachable rests on the renderer's source.

**The test:** when a test asserts that some output never appears, name the input that would produce it and confirm the test feeds that input, rather than trusting a broad run to wander into it. Arm it by restoring the configuration that allowed the output and watching the forcing case go red on every run, not on one run in sixteen. A sweep that passes against the broken build is not a guard, however many sequences it counts.

**Related:** "A test passes by avoiding the hard position" is the workbench memory this repeats. "Text drawn under --watch without replacing its control characters," above, is the same shape at a different sink: a fixture that never carried the value that breaks the code.

## A filter that decides which cases a check reads, measured on a representation the cases never take

Caught at Implement on dinah-603, 2026-09-26, by the implementer's own arming run, before the card left the column. The feature is a test that every frame of the terminal UI leaves the window's last column as an unstyled blank. It captures every frame's content, draws each into a cell buffer at the window's width, and checks the last cell of every row. During the sweep the window is resized ten columns narrower for a while, so a frame is checked at whichever of the two widths it fits.

**Wrong:** `if displayWidth(row) > width { return false }` inside the filter that picks the width. The frames carry their styling as SGR sequences, and `displayWidth` counts those bytes as columns, so every styled frame measured wider than either window and was skipped. The test logged "25 frames checked" because it counted the frames it had collected, not the ones it had read, and it stayed green when the layout was changed to fill the last column. The only frames it could have read were unstyled, and there were none.

**Right:** `if displayWidth(visible(row)) > width { return false }`, measuring what the terminal will show. Count the cases the check actually read, and fail when that count falls short of the cases collected, so a filter that drops cases is reported rather than absorbed.
