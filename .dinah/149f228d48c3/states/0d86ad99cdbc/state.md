...
2. **Block the card where it sits** when the spec is sound but carries an **unexpected** operator decision: a pending open question stamped `owner="operator"` or left unstamped, contested framing, a ruling not yet given. `block_card(card_id, kind="operator_decision", reason=<what needs deciding, posed so the operator can answer it without opening the card>)`. The block clears your claim and halts the card here, and only the operator lifts it, so do not call `move_card` afterwards. Never park such a card in Build Queue.
3. **Move to Operator Review/Ready** when the card produced something the operator has to accept before anything downstream commits to it. This is the **expected** case and it is not a block: the card is fine, it has simply reached a planned checkpoint. A UX sketch is the common one; an external interface, a change to how the board itself works, published copy and a customer-facing schema are the others. Spec should have named it in its move-note, and you are the backstop when it did not. Name the onward column in your move-note: Operator Review is off-lane and cannot work out where the card goes next.
4. **Promote to Build Queue/Ready** on a clean read with nothing left for the operator.

### Telling exit 2 from exit 3

Both end at the operator and they mean different things. A block says work stopped on a question nobody anticipated. Operator Review says work reached a checkpoint everybody expected.

The test for exit 3 is mechanical: **state in one line what acceptance criterion the operator's approval would create.** "The shipped command output matches the approved transcript on the card's branch" is such a line. If you cannot write one, there is nothing to approve, so this is not exit 3; either the card is clean (exit 4) or it carries a question (exit 2). Put that line in the move-note, because it is what the operator reads first and what Test verifies against later.

The error here is asymmetric. Routing a card to Operator Review that did not need it costs the operator one glance. Failing to route one means an implementation commits to a form nobody approved, with no criterion downstream to catch it. **When in doubt, take exit 3.**
