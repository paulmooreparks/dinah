...
func kindAllowlist(t *testing.T, root string) []kindExemption {
	...
	return []kindExemption{
		...
		{path: ownPrincipalFile(t, root), lines: 5, reason: "holds the predicates every reader asks, the shared kind grammar readColumn and column creation share, the kind a column created without one is born with, and which columns a columns view collapses when it declares nothing"},
