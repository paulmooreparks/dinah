...
func TestACommentViewCarriesAReferenceThatResolves(t *testing.T) {
	...
	detail, _, _, _, err := h.library.Show(&Request{Verb: "show", Card: card, Fields: "comments"})
	if err != nil {
...
func TestEveryChecklistItemCarriesAReferenceThatResolves(t *testing.T) {
	...
	detail, _, _, _, err := h.library.Show(&Request{Verb: "show", Card: card, Fields: "checklist"})
	if err != nil {
		t.Fatalf("show %s: %v", card, err)
	}
