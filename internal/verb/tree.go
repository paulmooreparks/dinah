...
func (l *Library) Tree(req *Request, chain []string, level string) (*Tree, error) {
	...
	kept, live, err := l.selection(req.Query, req.Actor, l.dayOf(req))
	if err != nil {
		return nil, err
	}
...
func (l *Library) workstreamContents(req *Request, entity *bench.EntityRef, level string) (*Tree, error) {
	...
	members, _, err := l.selection(narrowToReady(workstreamSelector(l.Bench, entity), req.ReadyOnly), req.Actor, l.dayOf(req))
	if err != nil {
		return nil, err
	}
