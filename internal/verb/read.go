...
//
// sawTier says whether some card with a landing passed the start hold and
// failed the tier test, and it is what lets a caller tell a queue with nothing
// in it from a queue whose work stands above the tier the caller resolved to.
// A queue holding only cards the hold withheld does not set it, because no
...
func (l *Library) Check(req *Request) (*CheckReport, error) {
	...
	// The scheduling-date stamp runs beside the applies_when stamp and on its
	// terms: without the confirmation it writes nothing, and it reads no
	// card.
	if req != nil && req.MigrateSchedule {
		migrated, err := l.Bench.MigrateSchedule(req.Confirm)
		report.MigratedSchedule = migrated
		if err != nil {
			return report, err
		}
	}
