...
// down, and the one extra is CountBlockingItems, which lists the checklist a
// second time because it opens each item's anchor. That is four at the
...
func TestOneCardViewMakesOneListingPerMountPlusTheBlockingRead(t *testing.T) {
	...
	// One listing per mount, plus the one CountBlockingItems makes over the
...
// Library.view calls reddens it too.
