...
// TestTheColumnViewCarriesTheDeclaredRejectTarget is the publication half of
// the declaration. ColumnView is the one read that enumerates what a column
// declares, and the MCP status tool serves it, so a column carrying reject_to
...
//
// The reference is published as written. A declaration naming no column opens
...
func TestTheColumnViewCarriesTheDeclaredRejectTarget(t *testing.T) {
	h := newHarness(t)
	...
	var declaring, ordinary *ColumnView
	for i := range views {
		switch views[i].ID {
		case aftercare:
			declaring = &views[i]
	...
	if declaring.RejectTo != "doing" {
		t.Errorf("the declaring column's view carries reject_to %q, the column declares \"doing\"", declaring.RejectTo)
	}
	if ordinary.RejectTo != "" {
		t.Errorf("a column declaring nothing carries reject_to %q", ordinary.RejectTo)
	}
	...
	if strings.Contains(string(bare), "reject_to") {
		t.Errorf("a column declaring nothing publishes the member anyway: %s", bare)
	}
}

// TestTheColumnViewPublishesAnUnresolvableRejectTarget pins the decision to
...
func TestTheColumnViewPublishesAnUnresolvableRejectTarget(t *testing.T) {
	h := newHarness(t)
	...
	for i := range views {
		...
		if views[i].RejectTo != "nowhere at all" {
			t.Errorf("the view carries reject_to %q, the column declares \"nowhere at all\"", views[i].RejectTo)
		}
		return
	}
	t.Fatalf("the declaring column did not come back: %+v", views)
}
...
func TestLegalMovesMarksTheDeclaredRejectTarget(t *testing.T) {
	t.Run("a declared target marks exactly that row", func(t *testing.T) {
		...
		response := h.mustDo(&Request{Verb: Move, Card: ref, Actor: "alka", Column: aftercare})
	...
	t.Run("a column declaring nothing marks no row", func(t *testing.T) {
		h := newHarness(t)
		ref := h.add("ordinary work")
		response := h.mustDo(&Request{Verb: Move, Card: ref, Actor: "alka", Column: aftercare})
		assertNoRejectRow(t, response.LegalMoves, "a column declaring nothing")
	})

	t.Run("a ref naming no column marks no row", func(t *testing.T) {
		h := newHarness(t)
	...
	t.Run("a ref naming the declaring column marks no row", func(t *testing.T) {
		h := newHarness(t)
...
// names the column given. Counting the marked rows rather than looking one up
...
func assertRejectRow(t *testing.T, moves []LegalMove, want string) {
	...
	for _, move := range moves {
		if move.Reject {
			marked++
			if move.Column != want {
				t.Errorf("the marked row names %s, wanted %s", move.Column, want)
			}
		}
	}
	if marked != 1 {
...
func assertNoRejectRow(t *testing.T, moves []LegalMove, what string) {
	...
	for _, move := range moves {
		if move.Reject {
			t.Errorf("%s marked the row naming %s", what, move.Column)
		}
	}
}
