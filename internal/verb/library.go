...
type Library struct {
	...
	Now func() time.Time
	// Observe, when set, is called by the containment projection as it reads a
	// collection's members: with ObserveList and the collection's path when it
...
// no-owner or not-operator refusal fills its harness-declared extra here,
// once, rather than at each of its call sites in this package.
