...
// TestTheReferenceTakingRosterIsFifteen derives the roster at the commit under
// test rather than reading it off a card, because three separate statements of
// this number, in the card's own framing and in its parent's prose, disagreed
// with the tables.
//
// It cannot pass vacuously. An empty parse yields nothing rather than fifteen,
// and the two declarations that carry the fact are compared against each other
// as well as against the named set, so a command declaring the guide on one of
// them and not the other is left out and the count fails.
func TestTheReferenceTakingRosterIsFifteen(t *testing.T) {
	roster := ReferenceTakingCommands()
	if len(roster) != 15 {
		t.Fatalf("the roster holds %d commands and it is fifteen: %s", len(roster), strings.Join(roster, " "))
	}
	want := []string{
		"archive", "attach", "attachments", "cite", "contents", "delete", "edit",
		"fail", "instructions", "path", "rename", "reopen", "resolve", "show", "verify",
	}
	...
	if byGuide != 15 || byParam != 15 {
		t.Errorf("the guide table names %d commands and the parameter table names %d, and both are fifteen", byGuide, byParam)
	}
