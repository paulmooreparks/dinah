...
// source, "x-dinah-vocabulary-source" on exactly the properties whose
// parameter declares one a head resolves when it runs, and "format":
// "duration" on exactly the properties whose parameter declares the duration
// placeholder.
//
...
func TestTheSchemaPublishesEachVocabularyAndDurationKeyExactlyWhereTheTableDeclaresIt(t *testing.T) {
	...
	for _, served := range tools {
		...
		for _, param := range verb.Params(served.command) {
			...
			wantEnum := hasVocabulary && set.Source == ""
		...
		for name, raw := range properties {
			...
			for _, key := range []string{"enum", "x-dinah-vocabulary-source", "format"} {
	...
	// Three counters rather than one, because a rule checked over a surface
	// where nothing triggers it passes for the wrong reason, and the three
	// keys are triggered by three different declarations.
	if enums == 0 {
		t.Error("no served property declares a fixed vocabulary, so the enum half of this test read nothing")
	}
	if sources == 0 {
