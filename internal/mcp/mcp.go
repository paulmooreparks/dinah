...
func call(root string, defaultLib *verb.Library, libraries map[string]*verb.Library, params json.RawMessage, profile string, memory *chainMemory) (map[string]any, error) {
	...
	payload, _ := answer.Run(tool.command, library, request)
...
func answerForest(root, tool string, defaultLib *verb.Library, request *verb.Request, build func(root, home string, req *verb.Request) (any, error)) (map[string]any, error) {
	...
	built, err := build(abs, home, request)
	if err != nil {
	...
	payload := answer.Wrap(map[string]any{member: built}, answer.ReadAffordances())
...
func workbenchesPayload(listed []bench.Candidate) (map[string]any, error) {
	...
	payload := answer.Wrap(map[string]any{"workbenches": listed}, answer.ReadAffordances())
	return textResult(payload)
}
...
func workbenchesDefaultOnly(defaultLib *verb.Library) (map[string]any, error) {
	...
	payload := answer.Wrap(map[string]any{
		"workbenches": []bench.Candidate{row},
		"unbounded":   true,
	}, answer.ReadAffordances())
	return textResult(payload)
}
...
func answerRefusal(request *verb.Request, refusal *contract.Refusal) map[string]any {
	response := answer.Refusal(request, refusal)
	encoded, err := answer.Encode(response)
	if err != nil {
		return map[string]any{
...
// the same parameter list the cli head composes its syntax from. The
// parameters are read by answer.Build, which the HTTP head shares; the
// identity this head reads off the call is added here.
