...
func TestAssignMarkerCoversEveryPublishedMarker(t *testing.T) {
	...
	if len(names) != 32 {
		t.Fatalf("swept %d markers, wanted 32 (twenty-four already wired, plus migrate-designations, rehearse and force-claims from dinah-472, plus migrate-applies-when from dinah-590, plus file-standing and migrate-raw-lines from dinah-593, plus plain and watch from dinah-288)", len(names))
	}
