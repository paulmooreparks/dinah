...
func TestAssignMarkerCoversEveryPublishedMarker(t *testing.T) {
	...
	if len(names) != 35 {
		t.Fatalf("swept %d markers, wanted 35 (twenty-four already wired, plus migrate-designations, rehearse and force-claims from dinah-472, plus migrate-applies-when from dinah-590, plus file-standing and migrate-raw-lines from dinah-593, plus explain from dinah-602, plus plain and watch from dinah-288, plus migrate-schedule from dinah-605, plus migrate-holds from dinah-608)", len(names))
	}
