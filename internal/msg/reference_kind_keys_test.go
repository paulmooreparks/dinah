...
// TestTheReferenceKindKeysReachEveryCatalogue enumerates the catalogue
// directory rather than listing the languages, so a ninth catalogue arriving
// fails this test instead of being missed, and it asserts the entry count so a
// catalogue silently skipped reports a number rather than passing.
//
...
// contract says of it.
//
// It follows TestTheCollectionKeysReachEveryCatalogue, which is the shape this
// repository already requires of a key family.
func TestTheReferenceKindKeysReachEveryCatalogue(t *testing.T) {
	keys := []string{
	...
	for _, file := range files {
		tag := strings.TrimSuffix(filepath.Base(file), ".json")
