Setup has written Dinah's section into AGENTS.md under {{base}}. Codex keeps its own configuration in TOML, which setup does not edit, so four things remain, and they are yours to do. Close any Codex session that is open first.

1. Register the `dinah` MCP server for this project. Add these lines to `.codex/config.toml` in {{base}}, creating the file and its directory if they do not exist:

       [mcp_servers.dinah]
       command = "dinah"
       args = ["mcp", "--tools", {{tools|toml}}, "--workbench", {{workbench|toml}}]

       [mcp_servers.dinah.env]
       DINAH_ACTOR = {{agent|toml}}
       DINAH_HARNESS = {{harness|toml}}
       DINAH_PROVIDER = {{provider|toml}}
