Setup has written the {{scope}}-scope configuration for Claude Code under {{base}}. Close any Claude Code session that is open before you run setup, because Claude Code writes some of these files itself when you change a setting. Four things remain, and they are yours to do.
...
3. At user scope, register the MCP server yourself, because Claude Code keeps user-scope servers in a file setup does not write:
       ...
       claude mcp add --env DINAH_ACTOR={{agent}} --env DINAH_HARNESS={{harness}} --env DINAH_PROVIDER={{provider}} --transport stdio --scope user dinah -- dinah mcp --tools {{tools}}
