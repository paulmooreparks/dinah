...
// for read.go's harnessExtra(req), occur twice with the identical shape), and
...
var unfollowedRefusalValues = map[string]string{
	...
	"internal/verb/read.go:harnessExtra(req)":       "harnessExtra (internal/verb/request.go) builds a map from one fixed key, contract.ValueHarness, to req.Harness, a value read off the request rather than a string literal written at any call site. It carries no English phrase at all, whether or not this check follows the call, which is what dinah-540's own review confirmed by reading the function rather than assuming it from the call shape. read.go hands it to RefuseWith at two sites (Whoami and Check's own owner gate), and this one entry excuses both, on this map's own documented grain: a key names a file and an expression rather than a line.",
	"internal/verb/reshape.go:harnessExtra(req)":    "The same function, the same reasoning, at Reshape's own no-owner and not-operator raise sites, which this one entry excuses together on the map's own grain.",
	"internal/httphead/commandlog.go:entry.Context": "The command log keeps the context member a refused answer carried, which the library built at its own raise sites, where this check reads it, and pageEntries rebuilds the refusal from it to render the entry's sentence. Following it here would mean following a struct field across the library boundary, which is dataflow this check does not do.",
	"internal/httphead/html.go:context":             "refusalOf rebuilds a refusal from the name, detail and context a refused answer or a view's refused section carried out of the library, to render the pages' error page and a view's refused section. The context was built at the library's own raise sites, where this check reads it, and following it here would mean following a struct field across the library boundary, which is dataflow this check does not do.",
	"cmd/dinah/view.go:section.RefusedContext":      "A view's refused section carries the refusal the query path raised, its Extra map included, out of the library as the section's refused_context member, and renderRefusedSection rebuilds the refusal from it to compose the section's line. The map's contents were built at the query path's own raise sites in internal/verb/query.go, where this check reads them, and following them here would mean following a struct field across the library boundary, which is dataflow this check does not do.",
}
