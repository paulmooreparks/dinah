...
func TestHelpBlockIsTheRatifiedSurface(t *testing.T) {
	...
	// The block lists forty-three commands, and every command the binary
	...
	if listed != 43 {
		t.Errorf("wanted forty-three listed commands, got %d", listed)
	}
}
...
func TestTheFlagSetsTheParserAcceptsAreDerivedFromTheParameterTable(t *testing.T) {
	wantValued := []string{
		...
		"lang", "max-depth", "operator", "priority", "query", "remint",
