...
func TestEnglishCommandListStartsEverySummaryAtOneColumn(t *testing.T) {
	...
	if summaries != 58 {
		t.Errorf("read %d command entries out of the block, want 58", summaries)
	}
	if wrapped != 21 {
		t.Errorf("%d entries wrapped across more than one line, want the twenty-one whose syntax is wider than half the window", wrapped)
	}
...
func TestAWideWorkbenchTitleStartsTheColumnsAfterItWhereTheyBelong(t *testing.T) {
	...
	got := runCLI(t, root, "list", "workbenches")
	if got.code != 0 {
