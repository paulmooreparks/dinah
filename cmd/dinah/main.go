...
func refusalSentence(r *msg.Renderer, name, verb, detail string, extra map[string]string) string {
	...
	if name == contract.UnknownField {
		return text + r.T("refusal.dinah.unknown-field.ordered", "instantField", instantField, "ordered", orderedOperators)
	}
...
// instantField is the one query field whose values rank, which is why it is the
// one that takes those four operators. It is read from the library rather than
// spelled a second time here, and it is bound at package scope because
// refusalSentence carries a parameter of its own named verb.
var instantField = verb.FieldAt
